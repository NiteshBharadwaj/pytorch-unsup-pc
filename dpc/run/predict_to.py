...
def build_model(model, input, global_step):
    ...
    for k in input.keys():        
        try:
            ...
            pass
    outputs = model(input, global_step, is_training=False, run_projection=True)
...
def compute_predictions():
    ...
    for k in range(6775,num_models):
        ...
        for view_idx in range(num_views):
            ...
            rgb = out["rgb_1"] if cfg.pc_rgb else None
            projs = out["projs"]
            ...
            if pose_student:
                proj_student_np, camera_student_np = model_student(input_image, model)
